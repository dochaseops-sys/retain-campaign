...
export function renderEmployeeDashboard() {
  ...
  container.innerHTML = `
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div class="modern-card p-8 sm:p-10 mb-8 bg-white border-slate-200">
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          
          <div class="flex items-center gap-4">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-sm" style="background-color: ${currentEmp.color || '#EF4444'}">
          ...
          <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 bg-[#FAFBF7] p-4 rounded-2xl border border-slate-200 shadow-inner">
            <div>
              <span class="text-[10px] font-black uppercase tracking-wider text-slate-500 block">Your Unique Referral Code</span>
              <span class="text-lg font-mono font-black text-slate-900 select-all">${currentEmp.referralCode}</span>
            </div>
            <button id="copy-code-btn" class="btn-dark-pill text-xs py-2 px-4 flex items-center justify-center gap-1.5 hover:scale-105 active:scale-95">
        ...
        <div class="mt-8 pt-6 border-t border-slate-100">
          ...
          <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center mb-6">
            <input type="text" readonly id="personal-link-input" value="${personalizedLink}" class="flex-1 bg-[#FAFBF7] border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 font-mono focus:outline-none select-all" />
            <button id="copy-link-btn" class="btn-retain text-xs flex items-center justify-center gap-2">
          ...
          <div>
            ...
            <div class="flex flex-wrap items-center gap-2.5">
              <a href="${whatsappShareUrl}" target="_blank" rel="noopener noreferrer" class="px-4 py-2.5 rounded-full bg-[#25D366]/10 hover:bg-[#25D366]/20 border border-[#25D366]/30 text-[#128C7E] text-xs font-bold transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                ${getSocialIcon('whatsapp', 'w-4 h-4 text-[#25D366]')}
                <span>WhatsApp</span>
              </a>
              <a href="${linkedinShareUrl}" target="_blank" rel="noopener noreferrer" class="px-4 py-2.5 rounded-full bg-[#0A66C2]/10 hover:bg-[#0A66C2]/20 border border-[#0A66C2]/30 text-[#0A66C2] text-xs font-bold transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                ${getSocialIcon('linkedin', 'w-4 h-4 text-[#0A66C2]')}
                <span>LinkedIn</span>
              </a>
              <a href="${xShareUrl}" target="_blank" rel="noopener noreferrer" class="px-4 py-2.5 rounded-full bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-800 text-xs font-bold transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                ${getSocialIcon('x', 'w-4 h-4 text-slate-900')}
                <span>X (Twitter)</span>
              </a>
              <a href="${facebookShareUrl}" target="_blank" rel="noopener noreferrer" class="px-4 py-2.5 rounded-full bg-[#1877F2]/10 hover:bg-[#1877F2]/20 border border-[#1877F2]/30 text-[#1877F2] text-xs font-bold transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
                ${getSocialIcon('facebook', 'w-4 h-4 text-[#1877F2]')}
                <span>Facebook</span>
              </a>
              <a href="${emailShareUrl}" class="px-4 py-2.5 rounded-full bg-slate-100 hover:bg-slate-200 border border-slate-300 text-slate-800 text-xs font-bold transition-all flex items-center gap-2 hover:scale-105 active:scale-95">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="modern-card p-6 border-slate-200">
          ...
          <span class="text-3xl sm:text-4xl font-black text-slate-900 block">${stats.totalPoints}</span>
          <span class="text-[11px] text-slate-500 font-medium">Verified scoring</span>
        </div>

        <!-- Leaderboard Position -->
        <div class="modern-card p-6 border-slate-200 bg-[#FAFBF7]">
          <div class="flex items-center justify-between text-slate-500 text-xs font-medium mb-1">
        ...
        <div class="modern-card p-6 border-slate-200">
          ...
          <span class="text-3xl sm:text-4xl font-black text-emerald-700 block">${stats.verifiedReferrals}</span>
          <span class="text-[11px] text-slate-500 font-medium">Confirmed followers</span>
        </div>
        ...
        <div class="modern-card p-6 border-slate-200">
          ...
          <span class="text-3xl sm:text-4xl font-black text-slate-700 block">${stats.pendingReferrals}</span>
          <span class="text-[11px] text-slate-500 font-medium">Awaiting verification</span>
        </div>
      ...
      <div class="modern-card p-8 border-slate-200">
        ...
        ${stats.recentActivity.length === 0 ? `
          <div class="text-center py-12 border border-dashed border-slate-200 rounded-2xl bg-[#FAFBF7]">
